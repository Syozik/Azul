...
export class Game {
    ...
    public constructor() {
        ...
        this.state.players = [new Player(1, this), new Player(2, this)];
    }
    ...
    public setState(newState: GameBackendState) {
        const hydratedPlayers = newState.players.map((playerState, idx) => {
            ...
            const player = new Player(source.playerNumber ?? idx + 1, this);
            player.pickedTiles = [...(source.pickedTiles ?? [])];
            player.coveredTiles = Object.fromEntries(
                Object.entries(source.coveredTiles ?? player.coveredTiles).map(
